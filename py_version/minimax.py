...
class board():
    ...
    def ai_turn(self, c_choice, h_choice):
        ...
        if depth == 9:
            ...
            state1 = state(self.get_board())
            move = state1.minimax(state1.get_board(), depth, COMP)
            x, y = move[0], move[1]
...
class state():
    ...
    def evaluate(self, state):
        ...
        if wins(self.get_COMP):
            score = +1
        elif wins(self.get_HUMAN):
            score = -1
        else:
            score = 0
    ...
    def __str__(self):
        return 'current state = ' + str(self.get_state())
...
def main():
    ...
    while len(board1.empty_cells()) > 0 and not board1.game_over():
        if first == 'N':
